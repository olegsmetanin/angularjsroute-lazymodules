                    ...
                    scope.$watch(srcExp, function(moduleName) {
                        ...
                        if (moduleName) {
							$loadOnDemand.load(moduleName, function() {
								...
								if (typeof(moduleConfig.templateCache) === 'undefined') {
									loadTemplate(moduleConfig.template, function(template) {
										var tc = {};
										$(template)
										.children('script[type="text/ng-template"]')
										.each(function(index, value) {
											var v = $(value);
											var template = v.text();
											tc[v.attr('id')] = template;
										});

										moduleConfig.templateCache = tc;

										childScope = scope.$new();
										var template = moduleConfig.templateCache[templateId];
